...
class AntiBlock : Hook(
    ...
    private val individualUnblockActivityViewModelAdjacentObject = "cf.s" // search for 'new DialogMessage(116, null'

    override fun init() {
        findClass(individualUnblockActivityViewModelAdjacentObject).hook("I", HookStage.BEFORE) { param ->
            GrindrPlus.shouldTriggerAntiblock = false
        }

        findClass(individualUnblockActivityViewModelAdjacentObject).hook("I", HookStage.AFTER) { param ->
