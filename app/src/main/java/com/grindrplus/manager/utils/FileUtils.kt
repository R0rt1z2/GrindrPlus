...
fun File.unzip(unzipLocationRoot: File? = null) {
    ...
    try {
        ZipFile(this).use { zip ->
            ...
            if (entries.isEmpty()) throw IOException("ZIP file is empty: $absolutePath")
            entries.forEach { entry -> extractZipEntry(zip, entry, rootFolder) }
        }
    } catch (e: Exception) {
        unzipLocationRoot?.takeIf { it.exists() }?.deleteRecursively()
        when (e) {
...
private fun extractZipEntry(zip: ZipFile, entry: java.util.zip.ZipEntry, rootFolder: File) {
