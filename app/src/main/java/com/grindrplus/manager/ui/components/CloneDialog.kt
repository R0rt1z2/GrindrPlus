...
fun CloneDialog(
    ...
    // Smelly use of readRemoteConfig(), this will do a bridge call every recomposition
    var existingClones by remember(Config.readRemoteConfig()) {
        mutableStateOf(AppCloneUtils.getExistingClones(context))
    }

    var selectedPackage by remember { mutableStateOf(existingClones.firstOrNull()) }
    val selectedNewPackage = !existingClones.contains(selectedPackage)

//    // this is probably unnecessary
//    LaunchedEffect(Unit) {
//        existingClones = AppCloneUtils.getExistingClones(context)
//        selectedPackage = existingClones.firstOrNull()
//    }

    var packageSelectMenuExpanded by remember { mutableStateOf(false) }
    var showNewPackageInfoDialog by remember { mutableStateOf(existingClones.isEmpty()) }


    var debuggable by remember { mutableStateOf(false) }
    var embedLSPatch by remember { mutableStateOf(!isLSPosed()) }
        ...
        Dialog(onDismissRequest = onDismiss) {
            Surface(
                ...
                Column(
                    ...
                    Box {
                        var mTextFieldWith by remember { mutableStateOf(0) }

                        OutlinedTextField(
                            // this dialog should not be shown if there's no package selected, so this should not happen
                            value = selectedPackage?.appName ?: "New clone...",
                            onValueChange = { },
                            readOnly = true,
                            modifier = Modifier
                                .fillMaxWidth()
                                .onGloballyPositioned { coordinates ->
                                    ...
                                    mTextFieldWith = coordinates.size.width
                                },
                            label = { Text("Select app") },
                            trailingIcon = {
                                IconButton(
                                    { packageSelectMenuExpanded = !packageSelectMenuExpanded }
                                ) {
                                    val icon = if (packageSelectMenuExpanded)
                        ...
                        DropdownMenu(
                            expanded = packageSelectMenuExpanded,
                            modifier = Modifier
                                .width(with(LocalDensity.current) { mTextFieldWith.toDp() }),
                            onDismissRequest = { packageSelectMenuExpanded = false }
                        ) {
                            existingClones
                                .filter { it.packageName != Constants.GRINDR_PACKAGE_NAME }
                                .map {
                                    DropdownMenuItem(
                                        text = { Text(text = it.appName) },
                                        onClick = {
                                            selectedPackage = it
                                            packageSelectMenuExpanded = false
                                        }
                                    )
                                }
                            ...
                            selectedPackage?.let {
                                if (selectedNewPackage) {
                                    ...
                                    DropdownMenuItem(
                                        text = { Text(text = "New clone...") },
                                        onClick = {
                                            packageSelectMenuExpanded = false
                                            showNewPackageInfoDialog = true
                                        }
                                    )
                                }
                            }
